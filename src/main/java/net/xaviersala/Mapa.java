...
public class Mapa {
  ...
  /**
   * El cavaller s'ha mogut i s'ha de mirar si conquereix noves comarques o no.
   * A més pot ser que conquereixi castells
   *
   * @param cavaller
   ...
   *
   * @throws InterruptedException Comprova si han cancel·lat el timer
   */
   ...
   *
   * @param guanyador
   *          Compte que dominarà totes les caselles
   */
  ...
  /**
   * Busca quin és el comte guanyador.
   *<p></p>
   * Només s'ha de cridar quan s'està segur de que s'ha acabat
   * @return comte que ha guanyat o bé null
   */
   ...
   *<p></p>
   * - Si ha arribat a un castell o sembla que se'n va de la pantalla li assigna
   * un nou destí
   *
   * @param comte
   *          Comte del que es mouen els cavallers
   * @return retorna si s'ha mogut algú o no.
   */
   ...
   * però les possibilitats de victòria depenen de la quantitat de comarques que
   * estiguin en poder d'aquest cavaller.
   *
  ...
  private void batalla(Cavaller primerCavaller, Cavaller segonCavaller) {
    ...
    LOG.info("** BATALLA!" + primerCavaller + ":" + comarquesPrimer + " vs "
                           + segonCavaller + ":" + comarquesSegon);
  ...
  private boolean foraDeRegio(Cavaller cavaller) {
    GRectangle colisio = cavaller.getPosicio().intersection(rectanglePais);
    return colisio.equals(cavaller.getPosicio());
  }

  /**
   * Ordena a un cavaller anar cap a un lloc (en principi només seran castells).
   *
   * @param cavaller
