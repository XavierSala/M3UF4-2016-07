...
public class Cavaller {
   ...
   *          imatge del cavaller
   * @param comte
   *          comte al que segueix el cavaller
   */
  ...
  public void setDesti(GRectangle on) {
    desti = on;
    double dx = (desti.getX() + desti.getWidth() * 0.5)
                    - (imatge.getX() + imatge.getWidth() * 0.5);
    double dy = (imatge.getY() + imatge.getHeight() * 0.5)
                    - (desti.getY() + desti.getHeight() * 0.5);
